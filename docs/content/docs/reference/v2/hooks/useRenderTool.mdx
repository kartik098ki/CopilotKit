...
`useRenderTool` registers a chat renderer for tool calls. You can target a specific tool name (with a typed Zod
`parameters` schema) or use a wildcard (`"*"`) fallback renderer.

This is part of the v2 tool rendering hook set with [`useComponent`](/reference/v2/hooks/useComponent),
