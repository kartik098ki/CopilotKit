...
<Callout type="warning">
  `useCopilotChat` is still supported, but we recommend migrating to [`useAgent`](/reference/v2/hooks/useAgent) from the
  v2 API.
</Callout>

`useCopilotChat` is a lightweight React hook for headless chat interactions. Perfect for controlling the prebuilt chat
...
<PropertyReference name="id" type="string">
  A unique identifier for the chat. If not provided, a random one will be generated. When provided, the `useChat` hook
  with the same `id` will have shared states across components.
</PropertyReference>
