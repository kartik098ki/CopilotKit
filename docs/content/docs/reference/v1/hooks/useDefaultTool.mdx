...
description:
  "The useDefaultTool hook enables rendering of a default UI which catches any tool that does not have a specific
  renderer."
---

<Callout type="warning">
  `useDefaultTool` is still supported, but we recommend migrating to
  [`useFrontendTool`](/reference/v2/hooks/useFrontendTool) from the v2 API.
</Callout>

`useDefaultTool` is a React hook that allows you to render custom UI for any tool call that doesn't have a specific
renderer
      ...
      <div className="p-4 border rounded my-2">
        ...
        {Object.keys(args).length > 0 && (
          <div className="mb-2">
            ...
            <pre className="text-xs bg-gray-100 p-2 rounded mt-1">{JSON.stringify(args, null, 2)}</pre>
          </div>
        )}
        ...
        {status === "complete" && result && (
          <div>
            ...
            <pre className="text-xs bg-gray-100 p-2 rounded mt-1">{JSON.stringify(result, null, 2)}</pre>
          </div>
        )}
      </div>
    );
