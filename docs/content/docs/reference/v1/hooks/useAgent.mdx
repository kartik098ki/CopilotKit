...
<Callout type="warning">
  `useAgent` is still supported, but we recommend migrating to [`useAgent`](/reference/v2/hooks/useAgent) from the v2
  API (`@copilotkit/react-core/v2`).
</Callout>

## Overview

`useAgent` is a React hook that returns an [AG-UI AbstractAgent](https://docs.ag-ui.com/sdk/js/client/abstract-agent)
instance. The hook subscribes to agent state changes and triggers re-renders when the agent's state, messages, or
...
function StateController() {
  ...
  return <button onClick={() => agent.setState({ ...agent.state, count: 1 })}>Increment</button>;
}
```
...
- **Automatic Re-renders**: Component re-renders when agent state, messages, or execution status changes (configurable
  via `updates` parameter)
