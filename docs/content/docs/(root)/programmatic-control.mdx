...
Programmatic control lets you run agents outside of a chat UI — trigger agent runs, access results, and manage state
entirely from code. Build agent-powered features that don't require a chat window.
...
export function AgentButton() { const { agent } = useAgent(); const { copilotkit } = useCopilotKit();
