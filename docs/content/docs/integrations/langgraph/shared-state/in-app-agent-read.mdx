...
You can also render the agent's state in the chat UI using `useRenderTool` or by accessing `agent.state` from
`useAgent`. The state is reactive and updates automatically.
