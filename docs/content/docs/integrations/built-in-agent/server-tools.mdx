...
Server tools are functions that run on your backend that the Built-in Agent can invoke. They're defined using
`defineTool()` with Zod schemas for type-safe parameters.
...
Throw errors or return error objects from your tool — the agent will see the error and can inform the user or try a
different approach:
...
By default, the agent performs a single step. If your agent needs to chain tool calls (e.g., search first, then create a
ticket), set `maxSteps`:
...
<Callout type="info">
  See [Advanced Configuration](/built-in-agent/advanced-configuration) for more options like `toolChoice`,
  `temperature`, and `providerOptions`.
</Callout>
