...
MCP (Model Context Protocol) servers provide additional tools and capabilities to your agent. The Built-in Agent
supports connecting to MCP servers via **HTTP** or **SSE** transports.
...
The HTTP transport also accepts optional `options` for advanced configuration of the underlying
