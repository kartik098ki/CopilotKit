...
By default, the agent performs a single generation step. Set `maxSteps` to allow the agent to call tools and then
continue reasoning:
...
With `maxSteps: 5`, the agent can call a tool, process the result, call another tool, and so on — up to 5 iterations.
This is essential for workflows where the agent needs to chain multiple tool calls.
...
<Callout type="info">
  Not all parameters are supported by every provider. For example, `topK` is supported by Google but not OpenAI.
...
Allow the frontend to override specific configuration at runtime. This is useful when you want users to switch models or
...
The full list of overridable properties: `model`, `toolChoice`, `maxOutputTokens`, `temperature`, `topP`, `topK`,
`presencePenalty`, `frequencyPenalty`, `stopSequences`, `seed`, `maxRetries`, `prompt`, `providerOptions`
