...
Frontend tools enable you to define client-side functions that your Pydantic AI agent can invoke, with execution
happening entirely in the user's browser. When your agent calls a frontend tool, the logic runs on the client side,
giving you direct access to the frontend environment.

This can be utilized to let [your agent control the UI](/pydantic-ai/frontend-tools),
[generative UI](/pydantic-ai/frontend-tools), or for Human-in-the-loop interactions.
