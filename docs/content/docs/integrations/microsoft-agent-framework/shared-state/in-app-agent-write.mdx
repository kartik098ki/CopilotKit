...
You can use this when you need to update agent state from your application — for example, changing user preferences,
toggling settings, or providing user input that affects agent behavior.
...
Depending on your agent implementation, you may choose to surface progress via tool responses or messages rather than
frequent state snapshots. See [Generative UI](/microsoft-agent-framework/generative-ui/tool-rendering) for recommended
patterns.
