...
Frontend tools enable you to define client-side functions that your Microsoft Agent Framework agent can invoke, with
execution happening entirely in the user's browser. When your agent calls a frontend tool, the logic runs on the client
side, giving you direct access to the frontend environment.

This can be utilized to let [your agent control the UI](/microsoft-agent-framework/frontend-tools),
[power generative UI](/microsoft-agent-framework/frontend-tools), or support Human-in-the-loop interactions.
