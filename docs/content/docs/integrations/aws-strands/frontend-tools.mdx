...
Frontend tools enable you to define client-side functions that your Strands agent can invoke, with execution happening
entirely in the user's browser. When your agent calls a frontend tool, the logic runs on the client side, giving you
direct access to the frontend environment.

This can be utilized to let [your agent control the UI](/aws-strands/frontend-tools),
[generative UI](/aws-strands/frontend-tools), or for Human-in-the-loop interactions.
...
Frontend tools have full access to the DOM - including your React component state. For example, you can have your agent
trigger re-renders via the `useFrontendTool` hook.
