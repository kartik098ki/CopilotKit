...
All Mastra Agents are stateful through working memory. This means that as your agent progresses, working memory is
preserved across the session. CopilotKit allows you to render this state in your application with custom UI components,
which we call **Agentic Generative UI**.
...
problem. The agent can store a draft in its working memory which is then rendered in the UI.
