...
The [`<CopilotKit>`](/reference/v1/components/CopilotKit) provider must wrap the Copilot-aware parts of your
application. For most use-cases, it's appropriate to wrap the `CopilotKit` provider around the entire app, e.g. in your
`layout.tsx`
  ...
  return (
    <html lang="en">
      <body>
        ...
        {/* [!code highlight:3] */}
        <CopilotKit publicApiKey="<your-copilot-cloud-public-api-key>">{children}</CopilotKit>
      </body>
    </html>
  );
}
