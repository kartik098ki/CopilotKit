...
<Callout type="info">Hover over the interactive UI elements below to see the available CSS variables.</Callout>

<CopilotKitCSS />
<InteractiveCSSInspector />

Once you've found the right variable, you can import `CopilotKitCSSProperties` and simply wrap CopilotKit in a div and
...
You can customize the fonts by updating the `fontFamily` property in the various CSS classes that are used in the
CopilotKit.
...
You can customize the icons by passing the `icons` property to the `CopilotSidebar`, `CopilotPopup` or `CopilotChat`
component.
