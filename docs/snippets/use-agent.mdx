...
Agents expose their state through the `agent.state` property. This state is shared between your application and the
agent - both can read and modify it.
...
You can subscribe to agent events using the `subscribe()` method. This is useful for logging, monitoring, or responding
        ...
        <div key={message.id}>
          ...
          {message.role === "assistant" &&
            message.toolCalls?.map((toolCall) => {
              const toolMessage = agent.messages.find((m) => m.role === "tool" && m.toolCallId === toolCall.id);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div className="p-6 bg-white rounded-lg shadow">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-3 h-3 rounded-full ${agent.isRunning ? "bg-yellow-500 animate-pulse" : "bg-green-500"}`}
            />
      ...
      <div className="p-6 bg-white rounded-lg shadow">
        ...
        {/* [!code highlight:3] */}
        <pre className="bg-gray-50 p-4 rounded text-sm overflow-auto">{JSON.stringify(agent.state, null, 2)}</pre>
      </div>
      ...
      <div className="p-6 bg-white rounded-lg shadow">
        ...
        <div className="space-y-3">
          ...
          {agent.messages.map((msg) => (
            ...
            >
              <div className="font-semibold text-sm mb-1">{msg.role === "user" ? "You" : "Agent"}</div>
              <div>{msg.content}</div>
            </div>
          ))}
        </div>
