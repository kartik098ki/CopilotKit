...
Docs assets are tracked with Git LFS. To ensure production builds always receive real asset bytes (not pointer files),
deploy docs through GitHub Actions using `.github/workflows/deploy_docs_vercel.yml`.
...
- Disable automatic Git-based deployments for the docs project and let the GitHub Actions workflow handle production
  deploys.
