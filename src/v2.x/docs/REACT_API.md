...
This document describes the public surface exported by `@copilotkit/react`. The package bundles React providers,
hooks, and chat UI components that sit on top of `@copilotkit/core` and the AG UI agent runtime. Import UI styles
from `@copilotkit/react/styles.css` when you render the bundled components.
...
All chat components rely on Tailwind utility classes baked into `@copilotkit/react/styles.css`. Import the CSS once
